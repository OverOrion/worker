...
pub fn make_config(cert: &[u8], key: &[u8]) -> WebSocketResult<Arc<rustls::ServerConfig>> {
...
fn load_certs(content: &[u8]) -> WebSocketResult<Vec<rustls::Certificate>> {
...
fn load_private_key(content: &[u8]) -> WebSocketResult<rustls::PrivateKey> {
	let rsa_keys = {
		let mut reader = BufReader::new(content);
